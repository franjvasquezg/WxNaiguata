...
f_menuOPC ()
{
   ...
   echo "[ 1] INC NGTA Debito Maestro (T${vpValRet_6}NA)             [ 5] Debito Maestro (${vpValRet_10})"
      ...
      then
         ...
         then
            ...
            do
               ...
               do
                     ...
                     if [ "${vARCHINC[$vADQIDX]}" -lt "1" ]
                     then
                        echo "No existe el Archivo de Reporte Debito Naiguata Maestro ${vpValRet_10}${SWTIPO} para el Adquiriente $vEntAdq" | tee -a $vFileLOG
                     ...
                     then
                        ...
                        then
                           ...
                           then
                              ...
                              f_fhmsg "Se envía archivo T464NA a platino2 (4200)" >> $vFileLOG 2>&1
                              f_fhmsg "Se envía archivo T464NA a platino2 (4200)"
                              scp -Bq $DIRIN/${vARCHINC[$vADQIDX]} $SSSH_USER@$FTP_HOSTXCOM:/file_transfer/${COD_AMBIENTEm}/${pEntAdqm}/file_out
                              vSTATT=$?
                              if [ "$vSTATT" != "0" ]
                              then
                                 tput setf 8
                                 echo "Error en la transferencia del archivo ${vARCHINC[$vADQIDX]} del adquiriente $pEntAdq al area de acceso de los Bancos, favor revisar" >> $vFileLOG 2>&1
                                 tput setf 7
                                 sqlplus -s $DB @$DIRBIN/alertacie INC MAESTRO-NGTA "Error_Transferencia_de_Archivo_$vArchDest_Adquiriente_$pEntAdq"
                              else
                                 echo
         ...
         #                    pg $DIRIN/$vArchDest
            ...
            do
               ...
               if [ "$vARCHINC" -lt "1" ]
               then
                  echo "No existe el Reporte Debito Maestro Naiguata SWCHDA $SWTIPO para el Adquiriente $pEntAdq" | tee -a $vFileLOG
                  sqlplus -s $DB @$DIRBIN/alertacie INC REPMAESTRO-NGTA "Reporte_de_Debito_Maestro_Adquiriente_$vEntAdq_No_Existe"
               fi
               if [ "$vARCHINC" -gt "1" ]
               then
                  echo "Existe mas de Un Archivo SWCHDA de Reporte Debito Maestro para el Adquiriente $pEntAdq, favor revisar" | tee -a $vFileLOG
