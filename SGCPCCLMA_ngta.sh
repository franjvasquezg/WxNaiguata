...
################################################################################
##
##  Nombre del Programa : SGCPCCLMA_ngta.sh
##                Autor : JMG
##       Codigo Inicial : 18/02/2008
##          Descripcion : Proceso Conciliacion de Maestro Naiguata
##
...
##  03/02/2020 FJV   3.00    Conciliacion Naiguata
##
################################################################################
...
dpNom="SGCPCCLMA_ngta"         # Nombre del Programa
...
## Cambio en nombre de archivo entrante NAIGUATA
vCodAmbi=`echo ${COD_AMBIENTE}`
if [ "${vCodAmbi}" = "PROD" ]; then
   vNomFile_Prefi="T464NA"
elif [ "${vCodAmbi}" = "CCAL" ]; then
   vNomFile_Prefi="T464NA"   #vNomFile_Prefi="470NA"  #caso de prueba IPR1302 NAIGUATA
elif [ "${vCodAmbi}" = "DESA" ]; then
   vNomFile_Prefi="T464NA"   #vNomFile_Prefi="470NA"
fi

##  Identificador del procesador adquirente Naiguata
if [ "${pEntAdq}" = "BM" ]; then
   vNomFile_IdAdq="0275"
elif [ "${pEntAdq}" = "BP" ]; then
   vNomFile_IdAdq="0313"
...
##vNomFile_SecA=`date +%Y`
##vNomFile_SecA=`echo ${vNomFile_SecA} | awk '{print substr($0,4,1)}'`
vNomFile_SecA=`echo ${pFecProc} | awk '{print substr($0,4,1)}'`
##echo "Secuencia Anio: ${vNomFile_SecA}"  #IPR1302

## Buscando Archivo de Datos con secuencia maxima
##vNomFile=${vNomFile_Prefi}_${vNomFile_IdAdq}_${vEntidadIdB}_*_${vNomFile_FeJul}_"conv"
##vNomFile=${vNomFile_Prefi}_${vNomFile_IdAdq}_${vEntidadIdB}_"0"_"062"_"conv33"   ##PRUEBA 464 IPR1302
vNomFile=${vNomFile_Prefi}_${vNomFile_IdAdq}_${vEntidadIdB}_${vNomFile_SecA}_${vNomFile_FeJul}_"conv"   ##PRUEBA 464 IPR 1302
vFileDIR=${DIRTMP}/${dpNom}${vpFH}.TMP
##ls -a ${DIRIN}/${vNomFile_Prefi}_${vNomFile_IdAdq}_${vEntidadIdB}_[0-9]_${vNomFile_FeJul}_conv 2>/dev/null 1>${vFileDIR}
...
vRet=`ORAExec.sh "exec :rC:=PQPMAESTRO.pf_Load_conciliacion_ngta('${pEntAdq}',TO_DATE('$pFecProc','YYYYMMDD'),'${vNomFile}');" $DB`
f_vrfvalret "$?" "Error al Ejecutar PQPMAESTRO.pf_Load_conciliacion_ngta. Avisar a Soporte."
