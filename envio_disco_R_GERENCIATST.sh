...
vRUTAWIN1="$vPrefijoWin\\${arch}"  #"$vPrefijoWin\\${arch}"
vRUTAWIN2="\\Procesos\\Compensacion Naiguata-MC\\${COD_AMBIENTE2}\\T461NA\\${arch}" ## $vpValRet_910\\$vArchDestB"
echo       
echo "vPrefijoWin  $vPrefijoWin"                                             ## $COD_AMBIENTE2   NUEVA VARIABLE DE AMBIENTE IPR1302
...
vCOPIAWIN=`ssh ${SFTP_USER}@${SFTP_IMC_NGTA} cmd.exe /R "R:\\MFESFTP\\transtmve $vRUTAWIN1 $vPrefijoWin $vRUTAWIN2 $USUARIOTMVE $CLAVETMVE"`
vSTATT=$?
if [ "$vSTATT" != "0" ]
then
    echo 
